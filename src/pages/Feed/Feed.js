...
class Feed extends Component {
  ...
  // updatePost = post => {
  //   console.log('in update from socket ii');
  //   console.log(post);
  //   this.setState(prevState => {
  //     const updatedPosts = [...prevState.posts];
  ...
  finishEditHandler = postData => {
    ...
    fetch('http://localhost:8080/post-image', {
      ...
      .then(fileResData => {
        ...
        let graphqlQuery = { 
          query: `
            mutation createNewPostRequest($title:String!,$content:String!,$imageUrl:String!){
      ...
      .then(resData => {
        if (resData.errors && resData.errors[0].status === 422) {
          throw new Error(
            "Validation failed. Please verify your email and password"
          )
        }
        if (resData.errors) {
